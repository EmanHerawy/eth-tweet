...
contract TweetRegistry {
	...
	function register(string name) public RegistrationDisabled IsRegistered ValidName(name) IsTaken(name) returns (int result) {
	...
	function unregister() public IsRegistered returns (string unregisteredAccountName) {
		...
		_addressToAccountInfo[msg.sender].name = "";
